...
public class SortingAlgorithms {
	...
	public SortingAlgorithms(int n) {
		...
		for (int i = 0; i < a.length; i++) {
			a[i] = (int) (Math.random() * Math
					.pow(10, MAXIMUM_NUMBER_OF_DIGITS));
		}
		// Initialize initialArray and make a copy of a in initialArray
		// (use System.arraycopy)
		initialArray = new int[n];
		System.arraycopy(a, 0, initialArray, 0, n);
	}
	...
	private void insert(int i) {
		int temp = a[i];
		int j;
		// shift all elements greater than temp by one position to the right
	...
	private void merge(int first, int mid, int last) {
		...
		// Copy temp in a
		// arraycopy copies temp in a: a[first] = temp[0],
		// a[first + 1] = temp[1], etc.
	...
	private void pickPivotAndSort(int first, int last) {
		if (first < last) {
			// pick the pivot
			int index = pickPivotLocation(first, last);
	...
	public static void main(String[] args) {
		while (true) {
			...
			System.arraycopy(sa.initialArray, 0, sa.a, 0,
					sa.initialArray.length);
			// Clock time just before the call
