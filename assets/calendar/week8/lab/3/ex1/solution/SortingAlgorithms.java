...
public class SortingAlgorithms {
    ...
    public SortingAlgorithms(int n)
    {
        ...
        // Create a
        a = new int[n];
        // Fill a with random integers (between 0 and maxNumber-1)
        double maxNumber = Math.pow(10,MAXIMUM_NUMBER_OF_DIGITS);
        for(int i=0; i<a.length; i++) a[i] = (int)(Math.random()*maxNumber);
        // Make a copy in initialArray
        initialArray = new int[n];
        System.arraycopy(a,0,initialArray,0,a.length);
    }
    ...
    public String toString()
    {
        ...
        for(int i=0; i<a.length-1; i++)
            out+=a[i]+", ";
        if (a.length>0) out+=a[a.length-1]+"}";
        return out;
    }
    ...
    private void divideInTwoSortAndMerge(int first, int last)
    {
        if (first<last)
        {
            ...
            // Sort a[first,...,mid] (using divideInTwo)
            divideInTwoSortAndMerge(first,mid);

            // Sort a[mid+1,...,last] (using divideInTwo)
    ...
    private void pickPivotAndSort(int first, int last)
    {
        ...
        {
            // pick the pivot
            int pivotLocation = pickPivotLocation(first,last);
            // Place the pivot at the start of the array
            swap(pivotLocation,first);
    ...
    public void radixSort()
    {
        ...
        ArrayList[] groups = new ArrayList[10];
        ...
        for(int d=0; d<MAXIMUM_NUMBER_OF_DIGITS; d++){
            ...
            // at position d
            int powerOfTen = 1;
            for(int i=0; i<d; i++) powerOfTen *= 10;

            for(int i=0; i<this.a.length; i++){
               int index = (this.a[i]/powerOfTen)%10;
               groups[index].add(new Integer(this.a[i]));
            }
            ...
            for(int i=0; i<groups.length; i++)
            {
                ...
                while(it.hasNext()){
                    a[index++] = ((Integer)it.next()).intValue();
                }
            }
        }
    }
